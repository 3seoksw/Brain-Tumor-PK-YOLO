...
def main_pt():
    ...
    for category in categories:
        args: arg_util.Args = arg_util.init_dist_and_get_args(category)
        ...
        # now try to load some checkpoint as model weight initialization;
        # this ONLY loads the model weights
        ...
        param_groups: List[dict] = get_param_groups(
            model_without_ddp,
            nowd_keys={"cls_token", "pos_embed", "mask_token", "gamma"},
        )
        opt_clz = {
        ...
        # try to resume the experiment from some checkpoint.pth;
        # this will load model weights, optimizer states, and last epoch (ep_start)
        ...
        if ep_start >= args.ep:  # load from a complete checkpoint file
            ...
            for ep in range(ep_start, args.ep):
                ...
                tb_lg.update(
                    rest_hours=round(remain_secs / 60 / 60, 2),
                    head="z_burnout",
                    step=ep,
                )
                tb_lg.flush()
