...
class Args(Tap):
    ...
    exp_dir: str
    # data_path: str = "~/Documents/github/Brain-Tumor-PK-YOLO/data/RSNA-MICCAI"
    data_path: str
    init_weight: str = ""  # use some checkpoint as model weight initialization;
    # ONLY load model weights
    resume_from: str = ""  # resume the experiment from some checkpoint.pth;
    # load model weights, optimizer states, and last epoch

    # SparK hyperparameters
    mask: float = 0.6  # mask ratio, should be in (0, 1)

    # encoder hyperparameters
    model: str = "V9back"  # resnet50
    input_size: int = 224
    sbn: bool = True
    ...
    commit_msg: str = (
        os.popen(f"git log -1").read().strip().splitlines() or ["[unknown]"]
    )[-1].strip()
    last_loss: float = 0.0
    cur_ep: str = ""
    remain_time: str = ""
                ...
                json.dump(
                    {
                        "name": self.exp_name,
                        "cmd": self.cmd,
                        "git_commit_id": self.commit_id,
                        "git_commit_msg": self.commit_msg,
                        "model": self.model,
                    },
                    fp,
                )
            ...
            json.dump(
                {
                    ...
                    "rema": self.remain_time,
                    "fini": self.finish_time,
                },
                fp,
            )
            fp.write("\n")
...
def init_dist_and_get_args(category: str):
    from utils import misc

    exp_dir = f"exp_{category}"
    data_path_suffix = "_t1wce_2_class/images"
    data_path = category + data_path_suffix
    data_path_prefix = "data/RSNA-MICCAI/"
    ...
    root = os.path.abspath(os.path.join(e, os.pardir, os.pardir, os.pardir))
    args.exp_dir = os.path.join(d, e)
    args.data_path = os.path.join(root, data_path)
