...
\usepackage{array, makecell, amsmath, amssymb, bm, multirow}
...
However, the original PK-YOLO design exhibits notable limitations, primarily stemming from a plane-shift domain gap, with its backbone pretrained exclusively on axial images, resulting in substantial performance disparities across planes.

To solve this limitation, we propose a modified model architecture, Mixture PK-YOLO.
In this method, three different backbones, each of which is pretrained on an anatomical plane (\textit{i.e.}, axial, coronal, or sagittal), are used in a frozen manner.
...
Although experimental results show that the proposed Mixture PK-YOLO doesn't surpass that of the original PK-YOLO, it significantly reduces the number of trainable parameters while effectively showing competent model performance.
The code is available at \href{https://github.com/3seoksw/Brain-Tumor-PK-YOLO}{https://github.com/3seoksw/Brain-Tumor-PK-YOLO}.
...
Inspired by the mixture-of-experts models, Mixture PK-YOLO employs three backbone models, compared to the PK-YOLO with one single backbone model, each is treated as an expert to its corresponding plane data, as shown in Figure~\ref{fig:model_comp}.
This approach is to capture each plane-specific visual cues for each backbone models.

Pretraining process of the SparK RepViT backbone models is the same with the original PK-YOLO model.
Using the RepViT \cite{repvit} model as a foundation, it applies SparK \cite{spark} technique to incorporate domain-specific knowledge.
During the SparK pretraining process, the model is fed with plane-specific brain tumor images to reconstruct the images with randomly masked patches.
Given this, each SparK RepViT backbone in Mixture PK-YOLO is equipped with specialized plane-specific brain tumor knowledge, acting as an expert for its corresponding plane data.
...
As shown in Figure~\ref{fig:model_comp}, router module is adopted to combine the outputs from the SparK RepViT backbone models, then feed the outputs to the auxiliary CBNet and YOLOv9 as PK-YOLO did.
Detailed visual illustration of the router module from the proposed Mixture PK-YOLO can be found in \ref{fig:router}.

We assume three anatomical MRI planes, such that $k \in \{1, 2, 3\}$ corresponding to the planes $\{\text{axial}, \text{coronal}, \text{sagittal}\}$.
Given the planes, each plane-specific SparK RepViT backbone expert is denoted as $F_k(X)$, where $X\in \mathbb{R}^{C\times H \times W}$.
In this work, we assume all three channels are used (\textit{i.e.}, $C=3$) and fix the image size to $H\times W = 320\times 320$.
Each backbone produces a set of multi-scale feature maps $F^{(n)}_k(X) \in \mathbb{R}^{C_n\times H_n\times W_n}$, $\forall n = 1, \cdots, 5$, representing an image representation with different dimension sizes for scale $n$.
For each scale $n$ and plane $k$, the spatial feature map is reduced to a channel vector, 

\begin{align}
  \tilde{F}^{(n)}_k(X) = AAP(F^{(n)}_k(X)) \in \mathbb{R}^{C_n},
\end{align}

\noindent where $AAP(\cdot)$ is adaptive average pooling layer. Then, the reduced feature maps are given to the score function that maps,

\begin{align}
  & z^{(n)}_k = g_n(\tilde{F}^{(n)}_k(X)) \in \mathbb{R} \\
  & \rightarrow \bm{z}^{(n)} = [z^{(n)}_1, z^{(n)}_2, z^{(n)}_3 ] \in \mathbb{R}^3.
\end{align}

\noindent In this work, the score function $g_n(\cdot)$ is replaced with two-layered simple linear layers with ReLU layer inplaced in between.
...
\noindent The weight $w^{(n)}_k$ decides which plane to give a higher importance than the other plane information.
...
Unlike original PK-YOLO, Mixture PK-YOLO completely freezes the pretrained backbone models to remain the pretrained knowledge on brain tumor images across the planes.
As PK-YOLO uses a single backbone model pretrained on axial data to capture the general knowledge of brain tumor, it is inevitable to unfreeze the backbone to adapt it to different plane data (\textit{i.e.}, coronal and sagittal data).
Whereas Mixture PK-YOLO doesn't require a fine-tuning process like the original PK-YOLO, since it employs three specialized backbone models, producing plane-specific brain tumor knowledge.
Not only this, it can significantly reduce the amount of parameters required for training by freezing the pretrained backbone models.
By freezing the SparK RepViT backbone, we are able to save almost 50\% of parameters to be trained, with PK-YOLO requiring 109.4 million parameters, compared to Mixture PK-YOLO requiring 55.5 million parameters.
Due to its reduction in the number of parameters to train, training overhead can be significantly reduced.

\begin{figure}
  \centerline{\includegraphics[width=0.6\textwidth]{images/router.png}}
  \caption{Router module from Mixture PK-YOLO to control the importance of plane-specific information.}
  \label{fig:router}
\end{figure}

\section{Experiments}

In this section, we evaluate our Mixture PK-YOLO model with the original PK-YOLO and the second best performed model, YOLOv8x \cite{yolov8x}.

% \begin{table}
%   \centering
%   \begin{tabular}{l | c | c | c | c | c | c}
%       \Xhline{2\arrayrulewidth}
%       Model & Plane & Image Size & Precision & Recall & mAP$_{50}$ & mAP$_{50:95}$ \\
%       \hline \hline
%       PK-YOLO & \multirow{3}{*}{Axial} & 640 & \textbf{0.858} & \underline{0.896} & \underline{0.947} & \textbf{0.681} \\
%       Mixture PK-YOLO & & 640 & \textbf{0.858} & 0.765 & 0.865 & 0.523 \\
%       Mixture PK-YOLO & & 320 & \underline{0.839} & \textbf{0.913} & \textbf{0.950} & \underline{0.630} \\
%       \hline
%       PK-YOLO & \multirow{3}{*}{Coronal} & 640 & \textbf{0.834} & \textbf{0.793} & \textbf{0.805} & \textbf{0.689} \\
%       Mixture PK-YOLO & & 640 & 0.457 & 0.719 & 0.495 & 0.265 \\
%       Mixture PK-YOLO & & 320 & \underline{0.460} & \underline{0.771} & \underline{0.549} & \underline{0.366} \\
%       \hline
%       PK-YOLO & \multirow{3}{*}{Sagittal} & 640 & \textbf{0.476} & \textbf{0.845} & \textbf{0.582} & \textbf{0.382} \\
%       Mixture PK-YOLO & & 640 & 0.449 & 0.574 & 0.401 & 0.213 \\
%       Mixture PK-YOLO & & 320 & \underline{0.452} & \underline{0.780} & \underline{0.455} & \underline{0.243} \\
%       \Xhline{2\arrayrulewidth}
%   \end{tabular}
% \end{table}
...
\begin{table}[t]
  \centering
  \caption{Model size comparison. The reduction rate indicates how much smaller 
  Mixture PK-YOLO is compared to each model.}
  \label{tab:models}
...
\begin{table}
  ...
  \caption{TODO: Put caption inside.}
  \begin{tabular}{l | c | c | c | c | c | c}
      \Xhline{2\arrayrulewidth}
      Model & Plane & Image Size & Precision & Recall & mAP$_{50}$ & mAP$_{50:95}$ \\
      \hline \hline
      YOLOv8x & \multirow{4}{*}{Axial} & 640 & \textbf{0.894} & 0.836 & 0.908 & \underline{0.656} \\
      PK-YOLO & & 640 & \underline{0.858} & \underline{0.896} & \underline{0.947} & \textbf{0.681} \\
      Mixture PK-YOLO & & 640 & \underline{0.858} & 0.765 & 0.865 & 0.523 \\
      Mixture PK-YOLO & & 320 & 0.839 & \textbf{0.913} & \textbf{0.950} & 0.630 \\
      \hline
      YOLOv8x & \multirow{4}{*}{Coronal} & 640 & \underline{0.672} & 0.650 & \underline{0.697} & \underline{0.524} \\
      PK-YOLO & & 640 & \textbf{0.834} & \textbf{0.793} & \textbf{0.805} & \textbf{0.689} \\
      Mixture PK-YOLO & & 640 & 0.457 & 0.719 & 0.495 & 0.265 \\
      Mixture PK-YOLO & & 320 & 0.460 & \underline{0.771} & 0.549 & 0.366 \\
      \hline
      YOLOv8x & \multirow{4}{*}{Sagittal} & 640 & 0.414 & \underline{0.782} & \underline{0.533} & \textbf{0.385} \\
      PK-YOLO & & 640 & \textbf{0.476} & \textbf{0.845} & \textbf{0.582} & \underline{0.382} \\
      Mixture PK-YOLO & & 640 & 0.449 & 0.574 & 0.401 & 0.213 \\
      Mixture PK-YOLO & & 320 & \underline{0.452} & 0.780 & 0.455 & 0.243 \\
      \Xhline{2\arrayrulewidth}
  \end{tabular}
\end{table}

\subsection{Ablation Study on Freezing Backbone Models}

We conducted an ablation study on freezing the three SparK RepViT backbone models for the proposed Mixture PK-YOLO.
Table~\ref{tab:ablation} shows the number of training parameters, and the overall model performance when the backbones are frozen and unfrozen.

While the Mixture PK-YOLO with unfrozen backbones achieves a slightly better performance than the one with frozen backbones in some metrics, the frozen model's overall performance exceeds the unfrozen model.
For instance, all of the metrics from sagittal plane from the frozen model show higher performance than those of the unfrozen model.
This is important, as the purpose of Mixture PK-YOLO is to reduce the performance gap between the axial plane and especially the sagittal plane.

Another advantage of freezing backbones is the significant reduction in the number of training parameters of the model.
Unfreezing the backbones means that the fine-tuning of three different SparK RepViT models must be done, which drastically increases the number of parameters.
Mixture PK-YOLO with unfrozen backbones requires 217.6 million parameters to train, which is about twice as many as the original PK-YOLO.
On the other hand, the model with frozen backbones requires 55.5 million parameters to train, which significantly reduces the total amount of parameters in the model while showing its competent performance throughout the planes.


\begin{table}
  \centering
  \caption{Ablation study on freezing backbones. Bold numbers show higher performance. Not only does freezing the backbones reduce the amount of training parameters, but also increases the overall model performance.}
  \label{tab:ablation}
  \begin{tabular}{c | c | c | c | c | c | c}
    \Xhline{2\arrayrulewidth}
    Frozen Backbones & \# Params (M) & Plane & Precision & Recall & mAP$_{50}$ & mAP$_{50:95}$ \\
    \hline \hline
    \multirow{3}{*}{\xmark} & \multirow{3}{*}{217.6} & Axial & \textbf{0.864} & 0.802 & 0.930 & \textbf{0.632} \\
                            & & Coronal & \textbf{0.483} & \textbf{0.781} & 0.544 & 0.354 \\
                                     & & Sagittal & 0.403 & 0.648 & 0.392 & 0.221 \\
    \hline
    \multirow{3}{*}{\cmark} & \multirow{3}{*}{55.5} & Axial & 0.839 & \textbf{0.913} & \textbf{0.950} & 0.630 \\
                            & & Coronal & 0.460 & 0.771 & \textbf{0.549} & \textbf{0.366} \\
                            & & Sagittal & \textbf{0.452} & \textbf{0.780} & \textbf{0.455} & \textbf{0.243} \\
    \Xhline{2\arrayrulewidth}
  \end{tabular}
\end{table}
...
\begin{table}[h]
  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}{|c|c|c|c|c|c|}
      ...
      Axial 320 & Ours & 0.839 & 0.913 & 0.950 & 0.630 \\ \hline
      Axial 640 & Ours & 0.858 & 0.765 & 0.865 & 0.523 \\ \hline
      ...
      Coronal 320 & Ours & 0.460 & 0.771 & 0.549 & 0.366 \\ \hline
      Coronal 640 & Ours & 0.457 & 0.719 & 0.495 & 0.265 \\ \hline
      ...
      Sagittal 320 & Ours & 0.452 & 0.780 & 0.455 & 0.243 \\ \hline
      Sagittal 640 & Ours & 0.449 & 0.574 & 0.401 & 0.213 \\ \hline
    \end{tabular}
  }
\end{table}
